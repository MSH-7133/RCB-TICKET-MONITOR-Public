...
# ===== TIMEOUTS =====
# ===== TIMEOUTS (Windows-optimized) =====
TIMEOUTS = {
    "page_load": 60,  # 60 seconds (Windows needs more time)
    "element_wait": 10,  # 10 seconds (increased for Windows stability)
    "otp_wait": 180,  # 3 minutes for OTP entry (extra buffer)
    "checkout_timer": 600,  # 10 minutes checkout timer
    "payment_timer": 360,  # 6 minutes payment timer
}

# NOTE: Bot's setup_driver() doubles page_load timeout automatically on Windows
# So actual page_load timeout = 60 * 2 = 120 seconds for ChromeDriver
